...
export default function CarCardDashboard(props: {car: Car}) {
    ...
    async function deleteCar() {
        try {
            await httpFetch<{ token: string }>(`cars/${props.car.id}`, true, {}, {
              method: 'DELETE',
            });
        ...
        <div className="card h-100 p-1 shadow-sm">
            <div className="card-body">
                ...
                <div className="d-flex justify-content-space-between">
                    <button className="w-100 btn btn-warning mt-2 me-2" onClick={() => navigate(`/dashboard/cars/${props.car.id}/edit`)}>Edit</button>
                    <button className="w-100 btn btn-danger mt-2" onClick={deleteCar}>Delete</button>
                </div>
            </div>
        </div>
    )
